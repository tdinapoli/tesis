...
def main():
    ...
    path = Path("/root/scripts/2024-09-02/lifetime_config.yaml")
    ...
    for curr in currs:
        print(f"Measuring lifetime with {curr=}")
        for wl in wls:
            print("At wavelength {wl} nm")
            home_mono(spec)
            df = measure_lifetime(wl=wl)
            itc.ld_output = False
            df.to_pickle(data_path/f"{wl}_{curr:.4f}.pickle")
