...
class M061CS02(abstract.Motor):
    ...
    def __init__(self, driver, steps = 200, angle = 0.0):
        self._driver = driver
        self._angle = angle
    ...
    def rotate_relative(self, angle, cw, change_angle = True):
        angle_done = 0.0
        self._driver.direction.state = cw
    ...
    def rotate_step(self, steps, cw, change_angle = True):
        self._driver.direction.state = cw
...
class Spectrometer(abstract.Spectrometer):
    ...
    def _angle_from_wl(self, wavelength, cw):
    ...
    def load_calibration(self, path): #wavelength
        ...
        for param in self._calibration:
            setattr(self, "_{}".format(param), self._calibration[param])
            ...
            setattr(self.__class__, param,
                    property(fget=lambda self: getattr(self, "_{}".format(param))))
    ...
    def calibrate(self):
        ...
        while cmd != 'q':
            ...
            if cmd == 'mxwl':
                ...
                print()
                for attr in self.CALIB_ATTRS:
                    print("{}: {}".format(attr[1:], getattr(self, attr)))
                print("calibration complete: {}".format(self.status()))
            elif cmd == 'wl':
